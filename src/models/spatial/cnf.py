...
class TimeVariableCNF(nn.Module):
    ...
    def __init__(self, func, dim, tol=1e-6, method="dopri5", solver=None, step_size = None, nonself_connections=False, energy_regularization=0.0, jacnorm_regularization=0.0):
        super().__init__()
        self.func = func
        self.dim = dim
        self.tol = tol
    ...
    def integrate(self, t0, t1, x, logpx, tol=None, method=None, norm=None, intermediate_states=0):
        ...
        if intermediate_states > 1:
            y = solution[3]
            _, _, _, _, logpy, energy, jacnorm = tuple(s[-1] for s in solution) # 取-1总算是明白了，因为是算tt[-1]的值，参考https://github.com/rtqichen/torchdiffeq/blob/master/examples/cnf.py
        else:
