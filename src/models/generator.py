...
class SeqGen(object):
    ...
    def compute_intensity_given_past(self,current_time,model,nlinspace=1):
        ...
        for i in range(len(last_time)):
            if not (last_time[i].item()>=0 and last_time[i].item()<self.args.t_max and current_time[i].item()>=0 and current_time[i].item()<self.args.t_max):
                print(last_time[i].item(),current_time[i].item())
                print('time error!')
                exit()
        ...
        hiddens = state_traj[1] # (1 + nlinspace, N, D); s[0]: ambda(t)
    ...
    def hidden_update(self,model, tpp_state, Lambda):
        ...
        updated_tpp_state = model.temporal_model.hidden_state_dynamics.update_state(event_time, tpp_state, cond=cond)
    ...
    def sample_one_event_attn(self, model,flag=True):
        ...
        if self.x_base is None:
            x = []
            for i in range(self.NN):
                xs = utils.gaussian_sample(model.spatial_model.z_mean,model.spatial_model.z_logstd) # 初始p(x0) N, T, D
                x.append(xs)
    ...
    def sample_location_attn(self, model):
        if len(self.spatial_locations[0]) == 0:
            ...
            #print('jump',self.prejump_hidden_states.shape)
            if isinstance(self.prejump_hidden_states,list):
                aux_state = torch.cat(self.prejump_hidden_states[1:-1],dim=1).to(self.device) 
            else:
                aux_state = self.prejump_hidden_states[:,1:]
                ...
                #print('prejump',len(self.prejump_hidden_states))
            ...
            if self.args.weekhour:
                ...
                #print(aux_state.shape,spatial_locations.shape)
                if aux_state.shape[1]!=spatial_locations.shape[1]:
                    print('error')
                    exit()
                inputs = [spatial_locations, aux_state, t_embed, aux_week, aux_hour, aux_type]
            else:
        ...
        x = x.reshape(T * N, -1).to(self.device)
    ...
    def sample_one_event(self,model):
        ...
        if len(self.spatial_locations[0]) == 0:
            for i in range(self.NN):
                ...
                self.spatial_locations[i].append(xs[0])

        before = (torch.cat([i[-1].unsqueeze(dim=0) for i in self.spatial_locations],dim=0).cpu()*self.args.S_std.cpu()+self.args.S_mean.cpu())
        
        xs, auxs = self.sample_location(model, aux_state=prejump_hidden_states) 

        after1 = (xs[:,:2].cpu()*self.args.S_std.cpu()+self.args.S_mean.cpu()).detach()

        if self.args.space_jump==1:
            obs_x = xs
            obs_t_origin = torch.tensor([[i[-1]] for i in self.event_times]).to(self.device) 
            obs_t = obs_t_origin + model.spatial_model.time_offset # add time_offset
            type_emb = model.type_emb(torch.tensor([i[-1] for i in self.event_types]).to(self.device))
            if self.args.weekhour_space_jump:
                cond_hour = model.hour_emb(((obs_t_origin % 1) * 24).long()).squeeze(dim=1)
                cond_week = model.week_emb((obs_t_origin % 7).long()).squeeze(dim=1)
                obs_type = type_emb.reshape(-1, type_emb.shape[1])
                cond = torch.cat([obs_t, obs_x, cond_week, cond_hour, obs_type, auxs[:, -model.spatial_model.aux_dim:]], dim=1)
            else:
                cond = torch.cat([obs_t, obs_x, auxs[:, -model.spatial_model.aux_dim:]], dim=1)
            
            xs = model.spatial_model.inst_flow(xs, reverse=True, cond=cond)

        for i in range(self.NN):
            if loc_mask[i]:
                self.spatial_locations[i].append(xs[i])
    ...
    def state_init(self):
        self.event_times, self.spatial_locations, self.event_types,  self.state_update, seq, self.intensity, self.intensity_event, self.state_save, self.init_state_save, self.x_base, self.prejump_hidden_states = [[] for _ in range(self.NN)], [[] for _ in range(self.NN)], [[] for _ in range(self.NN)], None, [[] for _ in range(self.NN)], None, None, [[] for _ in range(self.NN)], None, None, []

    def sample_seqs(self, model, seq_num):
        ...
        while t_temp:
            if self.args.model=='jumpcnf':
                temp = self.sample_one_event(model)
            elif self.args.model=='attncnf':
                temp = self.sample_one_event_attn(model)
    ...
    def sample_location(self, model, aux_state = None):

        if (np.array([len(i) for i in self.event_times])==1).all():
            start_time = torch.tensor(0.0).expand(self.NN).to(self.device)
        else:
            start_time = torch.tensor([i[-2] if len(i)>=2 else i[-1] for i in self.event_times]).to(self.device) + model.spatial_model.time_offset

        current_time = torch.tensor([i[-1] for i in self.event_times]).to(self.device) + model.spatial_model.time_offset

        xs = torch.cat([i[-1] for i in self.spatial_locations],dim=0).reshape(self.NN,-1,2)

        dlogps = torch.zeros(self.NN,xs.shape[1])

        xs = xs.reshape(-1, 2)
        dlogps = dlogps.reshape(-1).to(self.device)

        norm_fn = None

        if aux_state is not None:
            D_a = aux_state.shape[-1]
            auxs = aux_state[:,-1,:].reshape(-1, D_a)
            inputs = [xs, auxs]
            norm_fn = max_rms_norm([a.shape for a in inputs])
            xs = torch.cat(inputs, dim=1)

        before = (xs[:,:2]*self.args.S_std.to(self.device)+self.args.S_mean.to(self.device)).cpu().detach().numpy().tolist()

        xs, dlogps = model.spatial_model.cnf.integrate(start_time, current_time, xs, dlogps, norm=norm_fn) 
